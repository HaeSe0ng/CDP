extern "C" __global__ void default_function_kernel0(void* __restrict__ A, void* __restrict__ B, void* __restrict__ compute) {
  ...
  for (int k_outer = 0; k_outer < 16; ++k_outer) {
    ...
    for (int ax1_inner = 0; ax1_inner < 32; ++ax1_inner) {
      A_shared[((((((int)threadIdx.y) * 1024) + (ax1_inner * 32)) + ((int)threadIdx.x)))] = ((float*)A)[(((((((int)threadIdx.y) * 16384) + (ax1_inner * 512)) + (k_outer * 32)) + ((int)threadIdx.x)))];
    }
    #pragma unroll
    for (int ax1_inner1 = 0; ax1_inner1 < 32; ++ax1_inner1) {
      B_shared[((((((int)threadIdx.y) * 1024) + (ax1_inner1 * 32)) + ((int)threadIdx.x)))] = ((float*)B)[((((((((int)blockIdx.x) * 32768) + (((int)threadIdx.y) * 16384)) + (ax1_inner1 * 512)) + (k_outer * 32)) + ((int)threadIdx.x)))];
    }
    __syncthreads();
    for (int k_inner = 0; k_inner < 32; ++k_inner) {
      #pragma unroll
      for (int ax1 = 0; ax1 < 32; ++ax1) {
        A_shared_local[(ax1)] = A_shared[((((((int)threadIdx.y) * 1024) + (ax1 * 32)) + k_inner))];
      }
      #pragma unroll
  ...
  for (int i_inner_inner = 0; i_inner_inner < 32; ++i_inner_inner) {
    ...
    for (int j_inner_inner = 0; j_inner_inner < 2; ++j_inner_inner) {
      ((float*)compute)[((((((((int)threadIdx.y) * 524288) + (i_inner_inner * 16384)) + (((int)blockIdx.x) * 64)) + (((int)threadIdx.x) * 2)) + j_inner_inner))] = compute_local[(((i_inner_inner * 2) + j_inner_inner))];
    }
  }
}
