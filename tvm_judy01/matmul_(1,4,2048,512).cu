extern "C" __global__ void default_function_kernel0(void* __restrict__ A, void* __restrict__ B, void* __restrict__ compute) {
  ...
  for (int k_outer = 0; k_outer < 32; ++k_outer) {
    ...
    for (int ax1_inner = 0; ax1_inner < 2; ++ax1_inner) {
      A_shared[((((((int)threadIdx.y) * 32) + (ax1_inner * 16)) + ((int)threadIdx.x)))] = ((float*)A)[(((((((int)threadIdx.y) * 1024) + (ax1_inner * 512)) + (k_outer * 16)) + ((int)threadIdx.x)))];
    }
    #pragma unroll
    ...
    for (int k_inner = 0; k_inner < 16; ++k_inner) {
      ...
      for (int ax1 = 0; ax1 < 2; ++ax1) {
        A_shared_local[(ax1)] = A_shared[((((((int)threadIdx.y) * 32) + (ax1 * 16)) + k_inner))];
      }
  ...
  for (int i_inner_inner = 0; i_inner_inner < 2; ++i_inner_inner) {
    ((float*)compute)[(((((((int)threadIdx.y) * 4096) + (i_inner_inner * 2048)) + (((int)blockIdx.x) * 16)) + ((int)threadIdx.x)))] = compute_local[(i_inner_inner)];
  }
}
