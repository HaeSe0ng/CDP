extern "C" __global__ void default_function_kernel0(void* __restrict__ A, void* __restrict__ B, void* __restrict__ compute) {
  ...
  for (int k_outer = 0; k_outer < 64; ++k_outer) {
    __syncthreads();
    A_shared[(((int)threadIdx.x))] = ((float*)A)[(((k_outer * 8) + ((int)threadIdx.x)))];
    #pragma unroll
    for (int ax1_inner = 0; ax1_inner < 8; ++ax1_inner) {
      B_shared[(((ax1_inner * 8) + ((int)threadIdx.x)))] = ((float*)B)[(((((((int)blockIdx.x) * 4096) + (ax1_inner * 512)) + (k_outer * 8)) + ((int)threadIdx.x)))];
    }
    __syncthreads();
  ...
  ((float*)compute)[(((((int)blockIdx.x) * 8) + ((int)threadIdx.x)))] = compute_local[(0)];
}
