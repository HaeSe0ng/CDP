...
struct matmul_kernel_launch_cfg {
  ...
  __host__ __device__ matmul_kernel_launch_cfg(int bsz, int M, int N, int K) {
    if (bsz == 1 && M == 2048 && N == 1 && K == 512) {
    ...
    } else if (bsz == 1 && M == 16 && N == 2048 && K == 512) {
      gridDim = dim3(128, 1, 1);
      blockDim = dim3(16, 4, 1);
      func = batch_matmul_1_16_2048_512_kernel;
    } else if (bsz == 1 && M == 16 && N == 16384 && K == 512) {
      gridDim = dim3(256, 1, 1);
      blockDim = dim3(32, 1, 1);
      func = batch_matmul_1_16_16384_512_kernel;
      ...
      printf("batch_matmul: WRONG ARGS (bsz=%d, M=%d, N=%d, K=%d)", bsz, M, N,
             K);
    }
  }
};